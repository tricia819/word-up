...
function checkIfWordIsReal(word) {
    ...
    $.ajax({
        ...
        success: function(response) {
            ...
            for ( i = 0; i < model.wordSubmissions.length; i++ ) {
              var w =  model.wordSubmissions[i];
              if (w.word == word){
                w.isRealWord= theAnswer;
              }
            }

            // model.wordSubmissions.forEach(function (w) {
            //   if (w.word == word) {
            //     w.isRealWord=theAnswer;
            //   }
            // });
